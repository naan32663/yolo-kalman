...
def plot_one_box_label(x, img, color=None, label=None, line_thickness=None):
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    ...
    if label:
        tf = max(tl - 1, 1)  # font thickness
        text_line = label.split("\n")
        text_size, baseline = cv2.getTextSize(str(text_line), 0, fontScale=tl / 3, thickness=tf)
        for i, text in enumerate(text_line):
            if text:
                draw_point = [c1[0], c1[1] - (text_size[1] + 2 + baseline) * i]
               
                cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
                cv2.putText(img, label, draw_point, 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
